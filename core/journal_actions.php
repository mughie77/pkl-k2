    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            if ($action === 'check_in') {
                if (!$journal) {
                    $stmt = $pdo->prepare("
                        INSERT INTO internship_journals (student_id, journal_date, check_in_time, check_in_latitude, check_in_longitude, activities)
                        VALUES (:student_id, :journal_date, :check_in_time, :lat, :lon, '')
                    ");
                    $stmt->execute([
                        ':student_id' => $user_id,
                        ':journal_date' => $today,
                ...
                if ($journal && !$journal['check_out_time']) {
                    $stmt = $pdo->prepare("
                        UPDATE internship_journals
                        SET check_out_time = :check_out_time, check_out_latitude = :lat, check_out_longitude = :lon
                        WHERE id = :id
                    ");
                    $stmt->execute([
